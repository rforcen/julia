...
void QuickHull3D::computeMaxAndMin() {
  ...
  charLength = std::max<double>(max[1] - min[1], max[2] - min[2]);
  charLength = std::max<double>(max[3] - min[3], charLength);
  if (explicitTolerance == AUTOMATIC_TOLERANCE) {
    tolerance = 3 * DOUBLE_PREC *
                (std::max<double>(fabs(max[1]), fabs(min[1])) +
                 std::max<double>(fabs(max[2]), fabs(min[2])) +
                 std::max<double>(fabs(max[3]), fabs(min[3])));
  } else {
